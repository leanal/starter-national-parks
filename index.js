const submitHandler = (event) => {
  ...
  if (!hasErrors) {
    // Create an empty object - RENDER REFACTOR
    const park = {
...
const favoriteButtonClickHandler = (event) => {
  ...
  /* BEFORE RENDER REFACTOR
  const park = event.target.parentNode;
  park.style.backgroundColor = "#c8e6c9"; */
};

// function for sorting by name - RENDER REFACTORED
...
// function for handling the nameSorter click - RENDER REFACTORED
...
/* function to handle the ratingSorter click - before RENDER REFACTORED
const ratingSorterClickHandler = (event) => {
  event.preventDefault();
...
const main = () => {
  ...
  const main = document.querySelector("main"); // RENDER REFACTORED
  // const allBtns = document.querySelectorAll(".rate-button"); //BEFORE RENDER REFACTOR
  ...
  /* iterate the list of buttons and add an event handler to each - BEFORE RENDER REFACTOR
  allBtns.forEach((btn) => {
